...
class FLIGHT_CONTROLLER:
	...
	def __init__(self):
		...
		self.connection = mavutil.mavlink_connection('udp:localhost:14550',baud = 115200)
	...
	def payload_drop(self):
		...
		#self.connection.mav.command_long_send(self.connection.target_system, self.connection.target_component,mavutil.mavlink.MAV_CMD_DO_SET_SERVO,0,10,1000,0,0,0,0,0)
		print("payload releasing")
		time.sleep(7)
		#self.connection.mav.command_long_send(self.connection.target_system, self.connection.target_component,mavutil.mavlink.MAV_CMD_DO_SET_SERVO,0,10,1900,0,0,0,0,0)
		#msg = self.connection.recv_match(type = "COMMAND_ACK",blocking = True)
		#msg = msg.to_dict()
		#self.is_payload = (msg["result"] == 0)
		#if(self.is_payload):
			#rospy.loginfo("payload released")
		#elif(not self.is_payload):
			#rospy.loginfo("Ah oh")
...
def compute_waypoints(mav):
		...
		wps11[17].x_lat = wp_g[0][0]
		wps11[17].y_long = wp_g[0][1]
...
if __name__ == '__main__':
	...
	if (True):		
		...
		mav.set_Guided_mode()
		mav.payload_drop()
		print(mav.gps.x,mav.gps.y,mav.gps.z)
		#time.sleep(10)
