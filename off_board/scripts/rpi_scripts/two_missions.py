...
def compute_waypoints(mav):
        ...
        wp_g[5] = xy2latlon(wp02, origin)
        wp_g[6] = xy2latlon(wp03, origin)
        wp_g[7] = xy2latlon(wp04, origin)
        ...
        for j in range(1, len(wps22)):
            ...
            wps22[j].x_lat = wp_g[j-4][0]
            wps22[j].y_long = wp_g[j-4][1]
            wps22[j].z_alt = wp_g[j-4][2]
        wps22[0]=wps22[1]
        wps22[4].command = 21
...
if __name__ == '__main__':
    ...
    if (True):		
        ...
        while True:
            ...
            if count >= len(wps11)-1:
                break
            time.sleep(1)
        mav.wpClear()
        print("1st mission completed")
        mav.wpClear()	
        mav.set_mode("GUIDED")
        mav.wpPush(wps22)
        mav.set_mode("AUTO")
